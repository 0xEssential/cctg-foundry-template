...
contract EssentialPlaySession is EssentialERC2771Context {
    ...
    function verifyAuthorization(address signer, address beneficiary) external view returns (bool) {
        return
            _sessions[signer].authorized == beneficiary && _sessions[signer].expiresAt >= block.timestamp;
    }
